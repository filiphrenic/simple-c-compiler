...
public class LRParser {
    ...
    public void errorRecovery( ){
    	...
    	System.err.printf("readed %s it is in input text %s", currentSym.toString(), currentSym.getOriginalText());
    	while( input.elementAt(this.inputindex).getType() != SymbolType.SYNC ){
    		this.inputindex++;
    	}
    	currentSym = input.elementAt(this.inputindex);
    ...
    public void reduceAction( Production production ){
    	LRNode tmp = new LRNode(production.getLeftHandSide());
    	...
    	stackSymbol.push(new LRSymbol(production.getLeftHandSide()));
    	stackNode.push(tmp);
    	//put action
